...
#     "/etc/static/ssl/certs/nscacert.pem"
  ...
  system.activationScripts.addGitHubAccessToken.text = ''
  ...
  environment.etc."sudoers.d/10-nix-commands".text = ''
    ${user.accountName} ALL=(ALL:ALL) NOPASSWD: \
      ...
      ${coreutils}/bin/env nix-env -p /nix/var/nix/profiles/system --set /nix/store/*, \
      ${coreutils}/bin/env /nix/store/*/activate, \
