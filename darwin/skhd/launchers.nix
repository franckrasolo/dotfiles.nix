...
lcmd + lshift - c : sops -d ${user.dotfiles}/secrets/bootstrap.yaml | sed -n 1p | sed -E 's/opmk: //' | tr -d '\n' | pbcopy
lcmd + lshift - f : sops -d ${user.dotfiles}/secrets/bootstrap.yaml | sed -n 3p | sed -E 's/s2p2: //' | tr -d '\n' | pbcopy
