...
# zsh completion for the Smol Machines smolvm CLI (v1.12.0).
# Generated by Kimi K3 (max) from recursive `smolvm <subcommand> --help`
# output and https://github.com/smol-machines/smol/blob/main/docs/cli.md
#
...
_smolvm_machine_cmds=(
  ...
  'fork:Fork a running forkable machine into a new clone (CoW memory + disks)'
  'fork-release:Assign parameters and release one held fork-pool slot'
  ...
  case $state in
    ...
    args)
      case $words[1] in
        ...
        fork)         _smolvm_machine_fork ;;
        fork-release) _smolvm_machine_fork_release ;;
...
_smolvm_machine_run() {
  ...
  _arguments \
    $_smolvm_help_opt \
    '(--from -I --image)'{-I,--image}'[Container image, or a local source: docker save archive (./img.tar, - for stdin) or rootfs dir; optional with a Smolfile or for bare VM mode]:image:_files' \
    ...
    '--cpus[Number of virtual CPUs (default 4)]:N:' \
    '--mem[Memory allocation in MiB (default 8192)]:MiB:' \
    '--storage[Storage disk size in GiB]:GiB:' \
    '--overlay[Overlay disk size in GiB]:GiB:' \
    ...
    '--oci-cache[Cache the pulled OCI image on the host so repeat ephemeral runs skip the registry pull (registry images only)]' \
...
_smolvm_machine_exec() {
  ...
  _arguments \
    ...
    '--timeout[Kill command after duration]:duration:(30s 5m 1h)' \
    '(-d --detach)'{-i,--interactive}'[Keep stdin open for interactive input]' \
    '(-d --detach)'{-t,--tty}'[Allocate a pseudo-TTY (use with -i for shells)]' \
    '(-d --detach)--stream[Stream output in real-time]' \
...
_smolvm_machine_create() {
  ...
  _arguments \
    ...
    '*'{-v,--volume}'[Mount host directory or S3 bucket (repeatable)]:HOST|REMOTE\:GUEST[\:ro]:_files -/'
    '*'{-p,--port}'[Expose port from VM to host (repeatable)]:HOST\:GUEST:' \
    '--net[Enable outbound network access]' \
    '--net-backend[Select the networking backend]:backend:((tsi\:"Use libkrun TSI networking" virtio-net\:"Use virtio-net with the host-side smolvm network stack"))' \
    '--dns[Custom DNS resolver for the guest (implies --net)]:IP:' \
    '--network[Join a named inter-VM network (implies --net, virtio-net only)]:name:'
    ...
    '(-I --image)--from[Create machine from a packed .smolmachine artifact (no registry pull)]:artifact:_smolvm_smolmachine' \
    '*:: :->command' && return 0
...
_smolvm_machine_start() {
  _arguments \
    ...
    '--forkable[Start as a fork base: memfd-backed RAM (CoW-cloneable) plus a control socket]' \
    '--fork-pool-size[Plan a CUDA fork pool with this many runnable clones (implies --forkable)]:clones:' \
    '--cuda-vram-limit-mib[Override the automatic logical VRAM budget per golden/clone CUDA session (requires --fork-pool-size)]:MiB:' \
    '--proxy[Proxy URL used for the in-VM image pull]:URL:' \
    '--no-proxy[Comma-separated NO_PROXY list that bypasses the proxy during image pull]:list:'
}

_smolvm_machine_fork() {
  _arguments \
    $_smolvm_help_opt \
    '--golden[The running, forkable source machine to clone from]:machine:_smolvm_machine_names' \
    '(--name-prefix -n --name)'{-n,--name}'[Name for the new clone machine]:name:' \
    '--count[Number of clones to create from one snapshot (default 1)]:count:' \
    '(-n --name)--name-prefix[Name batch clones PREFIX-0 through PREFIX-(COUNT-1)]:prefix:' \
    '--parallel[Maximum number of clone boots in flight during a batch fork (default 4)]:count:' \
    '--wait-ready[Wait for smolvm-fork-ready in a single-clone fork (batch forks always wait)]' \
    '--hold[Keep each clone parked at the inherited forkpoint as an already-booted pool slot]' \
    '--ready-timeout[Maximum time to wait for the golden workload forkpoint (default 10m)]:duration:' \
    '--forkable[Make the clone itself forkable (memfd RAM + control socket)]' \
    '--share-weights[Share the golden CUDA weights with this clone instead of copying them]' \
    '*'{-e,--env}'[Per-fork parameter, delivered as /etc/smolvm/fork-env (repeatable)]:KEY=VALUE:' \
    '*'{-p,--port}'[Pin the clone inbound port forwards (repeatable); otherwise remapped to free ports]:HOST\:GUEST:' \
    '*--secret-env[Inject a per-fork secret from a host env var (repeatable)]:GUEST_VAR=HOST_VAR:_smolvm_secret_env' \
    '*--secret-file[Inject a per-fork secret from a host file (repeatable)]:GUEST_VAR=/abs/path:_smolvm_secret_file'
}

_smolvm_machine_fork_release() {
  _arguments \
    $_smolvm_help_opt \
    '(-n --name)'{-n,--name}'[Held clone to assign and release]:machine:_smolvm_machine_names' \
...
_smolvm_machine_delete() {
  _arguments \
    ...
    '--cascade[Also delete any clones forked from this machine (children before the base)]'
}
...
_smolvm_machine_update() {
  _arguments \
    ...
    '*'{-v,--volume}'[Add volume mount (repeatable)]:host dir:_files -/' \
    '*--remove-volume[Remove volume mount]:HOST\:GUEST:' \
    '*'{-p,--port}'[Add port mapping (repeatable)]:HOST\:GUEST:' \
    '*--remove-port[Remove port mapping]:HOST\:GUEST:' \
...
_smolvm_pack_create() {
  _arguments \
    ...
    '--cpus[Default number of vCPUs for the packed VM (default 4)]:N:' \
    '--mem[Default memory in MiB for the packed VM (default 8192)]:MiB:' \
...
_smolvm_pack_prune() {
  _arguments \
    $_smolvm_help_opt \
    '(--all)--keep[Number of cached extractions to keep (default 5)]:N:' \
    '(--keep)--all[Remove all cached extractions]' \
    '--dry-run[Show what would be removed without actually removing]'
}
